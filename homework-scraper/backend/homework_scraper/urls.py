...
def api_root(request):
    ...
    base_url = 'https://api.dovydas.space' if 'api.dovydas.space' in request.get_host() else ''
    return JsonResponse({
        'status': 'success',
        ...
        'endpoints': {
            ...
            'test': f'{base_url}/api/test/',
            'auth': f'{base_url}/api/auth/',
            'scraper': f'{base_url}/api/scraper/',
            'tasks': f'{base_url}/api/tasks/',
            'monitoring': f'{base_url}/api/monitoring/',
...
urlpatterns = [
    path('', api_root, name='root'),  # Changed to show API info at root
